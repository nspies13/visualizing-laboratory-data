...
bmp_data <- arrow::read_feather("../inputs/bmp_data.feather")
...
::: {.column width="50%"}
<br><br><br><br> - This pie chart highlights the proportion of HIL errors by location.<br><br> - The ***size*** of the pie chart represents the ***proportion*** of errors.<br><br> - The ***color*** represents the ***type*** of error.<br><br> - The ***transparency*** represents the ***clinical context*** for the error<br><br> <br>
:::
:::
...
1.  What makes this version better than the pie chart?

2.  What can still be improved?

### Answers

1.  This version is better than the pie chart because the **absolute and relative quantities are easier to interpret** in the stacked bar chart.

2.  Improvements can still be made in the **clarity of the labels and the color scheme**.
:::
:::
:::
...
1.  What improvements are made in this version?

2.  What can still be improved?

### Answers

1.  This version **improves clarity by directly labeling** the graph with the relevant annotations and removing the legend.<br><br>It also uses a **color-blind friendly** color palette that **invokes the types of errors the colors represent**.

2.  Improvements can still be in how easy it is to **compare across groups**.
:::
:::
:::
### Best
...
2.  What trade-offs are made for this improvement?

### Answers

1.  This version **simplifies the direct comparisons across contexts** by aligning them vertical across different facets.

2.  Breaking the bars into separate facets **fills up more space**.
:::
...
my_sf <- read_sf("~/git/educational_materials/DataViz/visualizing-laboratory-data-adlm/visualizing-laboratory-data/inputs/od_shapefile_illinois.shp")

```

## Example 1: Choosing the Appropriate Variable
...
gg_pop_diverg <- 
  ggplot(my_sf) +
    geom_sf(aes(fill = DP05_0001E), color = "black", linewidth = 0.1) +
    scico::scale_fill_scico(palette = "vikO", begin = 0.15, end = 0.85, trans = scales::sqrt_trans(), breaks = c(10000, 5000000), labels = c("Lowest", "Highest")) + 
    guides(fill = guide_colorbar(title ="Population", direction = "vertical")) + 
    theme_void() +
    coord_sf(expand = T, clip = "off") +
    theme(legend.position = c(1.1, 0.66), legend.title.position = "top")

## Make Absolute Count Map
gg_od_abs <- 
  ggplot(my_sf) +
    geom_sf(aes(fill = od_abs), color = "black", linewidth = 0.1) +
    scico::scale_fill_scico(palette = "vikO", begin = 0.15, end = 0.85, trans = scales::sqrt_trans(), breaks = c(10, 100000), labels = c("Lowest", "Highest")) + 
    guides(fill = guide_colorbar(title ="Events", direction = "vertical")) + 
    theme_void() +
    coord_sf(expand = T, clip = "off") +
    theme(legend.position = c(1.1, 0.66), legend.title.position = "top")

## Combine Them
ggpubr::ggarrange(gg_od_abs, gg_pop_diverg, ncol = 2, common.legend = F)
...
gg_od_pct <-  
  ggplot(my_sf) +
    ...
    scico::scale_fill_scico(palette = "vikO", begin = 0.15, end = 0.85, breaks = c(0.001, 0.026), labels = c("Lowest", "Highest")) + 
    guides(fill = guide_colorbar(title ="Event Rate", direction = "vertical")) + 
    theme_void() +
    theme(legend.position = c(1.1, 0.66), legend.title.position = "top")

## Combine Them
ggpubr::ggarrange(gg_od_pct, gg_pop_diverg, ncol = 2, common.legend = F)
...
gg_pop_sequential <- 
  ggplot(my_sf) +
    geom_sf(aes(fill = DP05_0001E), color = "black", linewidth = 0.1) +
    scico::scale_fill_scico(palette = "lipari", begin = 0.15, end = 0.85, trans = scales::sqrt_trans(), breaks = c(10000, 5000000), labels = c("Lowest", "Highest")) + 
    guides(fill = guide_colorbar(title ="Population", direction = "vertical")) + 
    theme_void() +
    theme(legend.position = c(1.1, 0.66), legend.title.position = "top")
...
gg_od_pct_seq <-  
  ggplot(my_sf) +
    geom_sf(aes(fill = od_pct), color = "black", linewidth = 0.1) + 
    scico::scale_fill_scico(palette = "bilbao", begin = 0.85, end = 0.15, breaks = c(0.001, 0.026), labels = c("Lowest", "Highest")) + 
    guides(fill = guide_colorbar(title ="Event Rate", direction = "vertical")) + 
    theme_void() +
    theme(legend.position = c(1.1, 0.66), legend.title.position = "top")

## Combine Them
ggpubr::ggarrange(gg_od_pct_seq, gg_pop_sequential, ncol = 2, common.legend = F)
...
## Exploring a Real-World Dashboard
...
Talking Points: - Who is the target audience? - What metrics are most relevant? - Does the current design acheive the goals? - Is the information clear, concise, and easy to understand? - What could be removed or added?
:::
:::
